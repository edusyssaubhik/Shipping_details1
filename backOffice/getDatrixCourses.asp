...
'certificateId = 4
'else
  certificateId = request.QueryString("certificationId")
'End IF
...
strCourses="SELECT * FROM "& Table &" WHERE country = 'United Kingdom'  AND certificateId = "& certificateId &" AND status = 'Available' and vendorId = '2' and (coursetype='Normal' or coursetype='Live') order by startdate,city desc"
...
If(CDate(arrAllCourses(6,rowCounter))-(Date())>=0) Then
						 
'If (IsNull(arrAllCourses(28,rowCounter)) OR arrAllCourses(28,rowCounter) <> "Corporate") Then

If (arrAllCourses(23,rowCounter) <> "Full" ) Then					
                       
str = str & "<option value="& arrAllCourses(0,rowCounter) &">"&arrAllCourses(27,rowCounter)&"-"& arrAllCourses(2,rowCounter) &" - "& arrAllCourses(1,rowCounter) &" : "& FormatDateTime(arrAllCourses(6,rowCounter),1) &" to "& FormatDateTime(arrAllCourses(7,rowCounter),1) &" for "
    if arrAllCourses(16,rowCounter)="" then
    days=Cint("0")
    'response.Write(days)
    end if
'	 If (CDate(arrAllCourses(3,rowCounter))-(Date()+arrAllCourses(16,rowCounter)+1) >= 0) Then
 ...
 ' str = str & " "& Currency_Format_Front &"&nbsp;"& FormatNumber(arrAllCourses(19,rowCounter)) &"&nbsp;"& Currency_Format_Back &""
  ...
  str = str & " "& Currency_Format_Front &"&nbsp;"& FormatNumber(arrAllCourses(32,rowCounter))&"&nbsp;"& Currency_Format_Back &""
