...
If addCourse = "Add Course" Then
 ...
 for i=0 to 9  
if i=0 then
rqStartDate1 	   =  Request.Form("startdate")
rqEndDate1		   =  Request.Form("enddate")
else
     x="startdate"&i
     y="enddate"&i
    rqStartDate1 	   =  Request.Form(x)
rqEndDate1		   =  Request.Form(y)
 
  end if
if rqStartDate1<>"" and rqEndDate1<>"" then    
  todayDate = (YEAR(now()) & "-" & (Month(now())) & "-" & (DAY(now()))) 
  StartDate = (YEAR(rqStartDate1) & "-" & (Month(rqStartDate1)) & "-" & (DAY(rqStartDate1)))
   EndDate = (YEAR(rqEndDate1) & "-" & (Month(rqEndDate1)) & "-" & (DAY(rqEndDate1))) 
