...
strQuery4  = "SELECT * FROM feedbacks WHERE Id = '" &rqEdit& "'"
Rs4.Open strQuery4,ConnObj


'Query To retrive competitor for dropdown to edit
strQuery = "select * from competitors WHERE country = '"&Country&"'"
  Rs5.Open strQuery,ConnObj	 	
...
%>
 <% If rqType = "Edit"  Then %>      
<body><div id="htMap">
 <table border="1" cellspacing="0" cellpadding="8" class="dbborder" Width="700px" align="center" >
   <form name="course" action="feedback-Recieved.asp" method="post">
        <tr><td colspan="2" class="HeaderLarge" align="center">  Edit FeedBack Form</td></tr>
        ...
        <tr><td width="50%">Name of student</td><td width="50%"><input type="text" name="Student_Name" size="30" readonly="readonly" value="<% = Rs4("Student_Name") %>"></td></tr>
       ...
       <span id="name" >
        ...
        <tr><td>How did you here about PROJstudy</td>
...
<tr><td>Other PRINCE2 training course providers considered</td>
...
<select name="other_training" multiple="multiple" style="width:143px">
 ...
 <% End If%>
        ...
        <tr><td>3 most important factors for considering PROJstudy?</td><td>
        ...
        <input Type="checkbox" name="moneyBack" <% If Rs4("Moneyback_Guarantee") = "Y" Then %>  checked="checked"<%End If%> value="Y"> Exam-Retake guarantee-Foundation<br>
	...
	<!--	<input Type="checkbox" name="Free_SixSigma" <% 'If Rs4("Free_SixSigma") = "Y" Then %>  checked="checked"<%'End If%> value="Y"> Free Six Sigma<br/>-->		
		<input Type="checkbox" name="guarante_class" <% If Rs4("guarante_class") = "Y" Then %>  checked="checked"<%End If%> value="Y">Guaranteed to Run Classes<br/>
		Others<br/><Textarea name="impFactors_Others" rows="5" cols="25"><% = Rs4("impFactors_Others") %></Textarea>
		</td></tr>
           <tr><td>Which Of the following did you do before enrolling for the classroom Training?(select all that apply)</td>
		   <td>
		<input Type="checkbox" name="Free_chapterTest" <% If Rs4("Free_chapterTest") = "Y" Then %> checked="checked"<%End If%> value="Y">Took the Free Chapter Test<br>
		<input Type="checkbox" name="Free_foundExam" <% If Rs4("Free_foundExam") = "Y" Then %> checked="checked"<%End If%> value="Y">Took the Free Simulated Foundation Exam<br>
		<input Type="checkbox" name="Free_guides" <% If Rs4("Free_guides") = "Y" Then %>  checked="checked"<%End If%> value="Y">Accessed the Free Study Guides and Podcasts<br>
		<input Type="checkbox" name="past_students"  <% If Rs4("past_students") = "Y" Then %>  checked="checked"<%End If%> value="Y">Interacted with past PROJstudy Students<br>
		<input Type="checkbox" name="reviews_internet"  <% If Rs4("reviews_internet") = "Y" Then %>  checked="checked"<%End If%> value="Y">Viewed PROJstudy reviews on the internet<br>
		<input Type="checkbox" name="visted_facebook"  <% If Rs4("visted_facebook") = "Y" Then %>  checked="checked"<%End If%> value="Y">Visted PROJstudy's Facebook page<br>
		<input Type="checkbox" name="twitter_updates"  <% If Rs4("twitter_updates") = "Y" Then %>  checked="checked"<%End If%> value="Y">viewed PROJstudy's Twitter updates<br>
		Other(Please specify)<br/><Textarea name="before_enrollothers" rows="5" cols="25"><% = Rs4("before_enrollothers") %></Textarea>
      ...
      Company Name:<input  name="companyName"  type="text" size="23" style="display:table-row"  value="<% = Rs4("companyName") %>"><br />
       ...
       <span id="name" >
        ...
        <tr><td>Improvement suggestions</td><td><Textarea name="suggestion" rows="5" cols="25"><% = Rs4("Improvement") %></Textarea></td></tr>
         <tr><td>Recommend PROJStudy to others?</td><td> <span id="feed" style="display:table-row;"> <input type ="radio" name="recommend" onclick="update1(this)"
...
<% Else %>   
 ...
 <tr><td>Which other certification(s) you plan to pursue in the near future</td>
 <td>
 <input Type="checkbox" name="cert_ITIL"  <% If Rs4("cert_ITIL") = "Y" Then %>  checked="checked"<%End If%> value="Y">ITIL<br/>
 <input Type="checkbox" name="cert_PMP"  <% If Rs4("cert_PMP") = "Y" Then %>  checked="checked"<%End If%> value="Y">PMP<br/>
 <input Type="checkbox" name="cert_Scrum"  <% If Rs4("cert_Scrum") = "Y" Then %>  checked="checked"<%End If%> value="Y">Scrum<br/>
<Textarea name="Pursue_Cert" rows="5" cols="25"><% = Rs4("Pursue_Cert") %></Textarea></td></tr>
