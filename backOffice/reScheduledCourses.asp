...
<%
 'Written By : Priyanka'
 'M0dified by: Priyanka'
 'Last UpaDated: 19/04/2011'
 'Description :'
...
<%
 ...
 Dim rqRescheduleCourse,str,rqResCourseID,rqVAT,rqFBdiscount
 ...
 If rqAction = "Cancel" Then
  ...
  rqStatusNumber = 3
   ...
   rqCanAmount =  Request.Form("CanAmount")
    ...
    strQuery1 =  "Update enrolledusers set status = '" & rqStatus & "',changestatusdateby = '" & ChangeStatusDateBy &"',statusnumber = '" & rqStatusNumber & "',res_can_mbg_amount = '" & rqCanAmount & "'  where id = '" & rqEnrollUserId & "'"
...
'ERG
 ChangeStatusDateBy = Now() & " by " & rqUserId
  ...
  rqStatusNumber = 5
    ...
    strQuery1 =  "Update enrolledusers set status = '" & rqStatus & "',changestatusdateby = '" & ChangeStatusDateBy &"',statusnumber = '" & rqStatusNumber & "',res_can_mbg_amount = '" & rqRefAmount & "'  where id = '" & rqEnrollUserId & "'"
...
' If rqType = "Re-Schedule" Then
 ...
 'Retrive Values From This Page'
  ...
  strQuery1 = "SELECT * FROM enrolledusers WHERE id = '"& rqEnrollUserId &"'"
  ...
  Do Until Rs.EOF 
   ...
   rqModeofPayment   =  Rs("modeofpayment")
...
' If rqResCourseID <> "" Then
	...
	'SQL Query For Retrieve The Prices Displaying in The Step Two'
	 ...
	 strQuery2 = "SELECT * FROM course Where courseid = '" & rqResCourseID & "'"
	...
	End If
	  ...
	  strQuery3 = "INSERT INTO enrolledusers (firstname,lastname,email,nameofemployeer,phoneno,coursedetails,coursedate,amount,dateofenrollment,paybefore,status,checkreceived,checkreceiveddate,checkreceivedby,PRINCE2Manual,PRINCE2price,statusnumber,courseid,discountpercentage,res_can_mbg_amount,VAT,street,city,state1,postalCode,speReq,modeofpayment,"
	  IF rqFBdiscount <> "" Then
	  strQuery3 = strQuery3 & "FBdiscount,"
	  End IF
	   strQuery3 = strQuery3 &"country)"
	 ...
	 strQuery3 = strQuery3 & "'" & rqModeofPayment & "',"
	  IF rqFBdiscount <> "" Then
	 ...
	 strNewUserId = "SELECT max(id) as EnrollId FROM enrolledusers"
...
'Adding backoffice Comments'
	 ...
	 strComments = "SELECT * FROM enrolledusercomments WHERE enrolluserid = '"& rqEnrollUserId &"'"
...
'Adding US backoffice Comments'
	 ...
	 strUSComments = "SELECT * FROM enrolledusercomments WHERE enrolluserid = '"& rqEnrollUserId &"'"
	 ...
	 strQuery1 =  "Update enrolledusers set status = 'Re-Schedule',changestatusdateby = '" & ChangeStatusDateBy &"',statusnumber = '2'  where id = '" & rqEnrollUserId & "'"
	 ...
	 strBody = "<table width=""100%"" bordrer=""1"" bgcolor=""#CCCCCC"" style=""margin:0px;0px;0px;0px;""><tr><td height=""45px""> </td></tr><tr><td><table width=""600"" border=""0"" align=""center"" cellpadding=""2"" cellspacing=""12"" bgcolor=""#FFFFFF""><tr><td width=""600px"" bgcolor=""#FFFFFF"">" 
						...
						If City = "Live" Then
						strBody= strBody &"class"
						
						ElseIf City = "Live" Then
						
						strBody= strBody &"classroom program"
						
						End IF
						strBody= strBody &" scheduled on <u>"&FormatDateTime(StartDate,1)&" to "&FormatDateTime(EndDate,1)&"</u>.<br><br> Best Regards,<BR>Customer Support<BR><a href=""http://www.PROJstudy.com"" target=""_blank"">www.PROJstudy.com</a><br><BR>Email: marketing@projstudy.com  <BR></font></td></tr></table></td></tr><tr height=""30px""><td> </td></tr></table></td></tr><tr height=""45px""><td> </td></tr></table>"
	...
	objCDOMail.Subject = "Reschedule PROJstudy Classroom Course"
...
<table border="1" width="1000px" height="400px" class="dbborder" cellspacing="0">
 <tr>
   <td colspan="3" height="35"><div align="center"><font color="#00B0EC" face="Arial" size="4">Welcome to <span style="color:#FF3300"><% = Session("country") %></span> PROJstudy Back Office</font></div></td>
 </tr>
 <tr>
...
'Retrive The CourseDates Page'
 ...
 If session("country") <> "" Then
 	...
 	strQuery4 = "SELECT * FROM course WHERE country = '"& session("country") &"' AND startdate >= '"& Now() &"' AND  vendorId <> '2' ORDER BY startdate desc"
...
'response.Write(strQuery4)
 Rs.Open strQuery4,ConnObj 
  ...
  %>
<form action="reScheduledCourses.asp" method="post" onSubmit="return Form_Validator1(this)">
...
<% ElseIf rqType = "ERG" Then %>
 <form action="reScheduledCourses.asp" method="post" onSubmit="return Form_Validator2(this)">
 <table border="0" width="77%" cellpadding="2" cellspacing="2" >
 <input type="hidden" name="enrollUserId" value="<% = rqEnrollUserId %>" />
 <input type="hidden" name="changestatus" value="<% = rqType %>" />
  <input type="hidden" name="CourseId" value="<% = rqCourseId %>" />
