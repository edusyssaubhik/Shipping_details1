...
If rqType = "Add New Query" Then
	...
	strPayment =  "Insert into paymentDecline (TypeofCourse,Name,EmailID, city, Country, InvoiceDate, datePayment_dec,FailureReason,Amount,Addedby,AddedDate,PayProcess_name,Status, ContactNo, Currency_type, Website, OrderNo) VALUES ('"&rqTypeofCourse&"', '"& rqName &"','"& rqEmailID &"','"& rqcity &"','"& rqCountry &"','"& rqInvoiceDate &"','"& rqdatePayment_dec &"','"& rqFailureReason &"', '"& rqAmount &"', '"& Session("UserID") &"', '"& now() &"', '"& rqPayProcess_name &"','1', '"&rqContactNo&"', '"&rqCurrency_type&"', '"&rqWebsite&"', '"&rqOrderNo&"')"
	
	
	'Response.Write(strPayment)
	objRs.Open strPayment,ConnObj
	'Response.Write(strPayment)
