...
'Response.write(rqCourseId)
  ...
  strQuery = "Select Distinct(country) From city"
' Response.write(strQuery)
  ...
  strQuery3 = "SELECT instructorid, firstname,lastname from instructor WHERE country = '"& session("country") &"'"
'Response.write(strQuery3)
  ...
  End If	  
		...
		strQuery2 = "select * from competitors  WHERE country = '"&rqCountry&"'"
		...
		strQuery6 = "SELECT distinct(c.courseId), c.city, c.startdate, c.enddate from course c, feedbacks f WHERE c.country = '"&rqCountry&"' AND  startdate < '"& Now() &"' AND status <> 'Cancelled'  and c.courseId = f.Course_ID group by c.courseId,c.city,c.startdate,c.enddate order by startdate desc"
	  ...
	  If rqCourseId <> "" Then		    
	  	strQuery1 = "select * from feedbacks where Course_ID ='"&rqCourseId&"'"
		...
		strQuery7 = "SELECT city, startdate, enddate from course WHERE courseID='"&rqCourseId&"'"
...
If rqfromdate <> "" Then 
		...
		strQuery1 = "select f.* from course c,feedbacks f,enrolledusers u where c.courseid = f.Course_ID and c.startdate between '"&rqfrom &"' and '"&rqto&"' and u.id = f.Enroll_ID and c.country='"&rqCountry&"' "
		'Response.write(strQuery1)		
		Rs1.Open strQuery1,ConnObj
		If Not Rs1.EOF Then
		arrQueries = Rs1.getRows
...
'Query to retrive faculty name
		strQuery5 = "select firstname,lastname from instructor where instructorid ='"&rqfacultyid&"'"
	...
	Session("rqname") =rqname 
		...
		strQuery1 = "select f.* from course c,feedbacks f,enrolledusers u where c.courseid = f.Course_ID and c.startdate between '"&Reporfrom&"' and '"&reportto&"' and u.id = f.Enroll_ID and c.instructor = '"&rqname&"'"
		'Response.write(strQuery1)   
		Rs1.Open strQuery1,ConnObj
		If Not Rs1.EOF Then
		arrQueries = Rs1.getRows
