...
'creating record set 
 ...
 'query to retrive course details depending on CourseId and today's Date
	strQuery = "SELECT courseId, city, startdate, enddate, coursetype, coursedates,instructor from course WHERE country = '"&Country&"' AND  startdate < '"& Now() &"' AND status <> 'Cancelled' order by startdate desc"
...
If rqCourseId <> "" Then
   strQuery2 = "SELECT id,firstname,lastname,courseid FROM enrolledusers WHERE (checkreceived = 'Yes' OR us_status = 'Active' OR us_status = 'P.O Received' OR us_status = 'Under Process') AND courseid = '"& rqCourseId &"'"
...
strQuery5 ="SELECT courseid,city,startdate,enddate,instructor from course where courseid = '"& rqCourseId &"' "
...
strQuery4 = "select * from competitors  WHERE country = '"&Country&"'"
...
<div style="overflow-y:scroll; overflow-x:hidden ">
     <table width="100%" border="0"  cellspacing="0" cellpadding="0" id="tHeader" style="display:block; border-collapse:collapse;">
              ...
              <th colspan="5" style="width:650px" scope="col">How did you hear about PROJstudy</th>
              <th rowspan="2" style="width:200px" scope="col">Other PMP training course providers considered</th>
              <th colspan ="8" style="width:950px" scope="col">3 most important factors for considering PROJstudy? </th>
			  <th colspan ="8" style="width:1300px" scope="col">Which Of the following did you do before enrolling for the classroom Training?(select all that apply)</th>
              ...
              <th rowspan="2" style="width:250px" scope="col">Recommend PROJstudy to others?</th>
			  <th rowspan="2" style="width:250px" scope="col">Name any other certification(s) that you have completed</th>
			  <th colspan="4" style="width:550px" scope="col">Which other certification(s) you plan to pursue in the near future</th>			  
             ...
             <th style="width:100px" scope="col">Exam-Retake guarantee-Foundation</th>
			...
			<!-- <th style="width:100px" scope="col">Free Six Sigma</th>-->
			 <th style="width:100px" scope="col">Gaurantee to Run Classes</th>
		     <th style="width:250px" scope="col"> Others</th>
			 
			<th style="width:150px" scope="col">Took the Free Chapter Test</th>
			<th style="width:150px" scope="col">Took the Free Simulated Foundation Exam</th>
			<th style="width:150px" scope="col">Accessed the Free Study Guides and Podcasts</th>
			<th style="width:150px" scope="col">Interacted with past PROJstudy Students</th>
			<th style="width:150px" scope="col">Viewed PROJstudy reviews on the internet</th>
			<th style="width:150px" scope="col">Visted PROJstudy's Facebook page</th>
			<th style="width:150px" scope="col">viewed PROJstudy's Twitter updates</th>
			<th style="width:250px" scope="col">Other(Please specify)</th>
					
			<th style="width:100px" scope="col">ITIL</th> 
			<th style="width:100px" scope="col">PMP</th> 
			<th style="width:100px" scope="col">Scrum</th> 
			<th style="width:250px" scope="col">Others</th> 
		 ...
		 <div style="overflow-y:scroll; height:300px ;">
		  <table  width="100%" border="0"  cellspacing="0" cellpadding="0"  id="tBody" style="display:block; border-collapse:collapse;">
           <%
		   ...
		   'Sql Query To Retrive values from the table if Enroll_ID and Course_ID eixts 
		    strQuery3 = "SELECT * FROM feedbacks WHERE Course_ID = '"&arrAllQueries(3,enrollrowcounter)&"' and Enroll_ID = '"&arrAllQueries(0,enrollrowcounter)&"' "
	...
	<tr align="center">     
            ...
            <td style="width:200px"> <% =Rs3("Other_Training_Providers")%></td>
            <%
...
'			End If			
	...
	<!--		<td style="width:100px"> <%' =Rs3("Free_SixSigma")%></td>-->
		    ...
		    <% 
			If Rs3("guarante_class") = "Y" then 
			count12 = count12 + 1
			
			End If			
			%>
			<td style="width:100px"> <% =Rs3("guarante_class")%></td>
			
			<td style="width:250px"> <Textarea name="impFactors_Others" rows="4" cols="25" readonly="readonly"><% =Rs3("impFactors_Others")%></Textarea></td>
			
			
			<!--'before enrolling for the class room training-->
			<td style="width:150px"> <% =Rs3("Free_chapterTest")%></td>
			<td style="width:150px"> <% =Rs3("Free_foundExam")%></td>
			<td style="width:150px"> <% =Rs3("Free_guides")%></td>
			<td style="width:150px"> <% =Rs3("past_students")%></td>
			<td style="width:150px"> <% =Rs3("reviews_internet")%></td>
			<td style="width:150px"> <% =Rs3("visted_facebook")%></td>
			<td style="width:150px"> <% =Rs3("twitter_updates")%></td>
			<td style="width:250px"> <Textarea name="before_enrollothers" rows="4" cols="25" readonly="readonly"><% =Rs3("before_enrollothers")%></Textarea></td>
            ...
            <td style="width:250px"> <% =Rs3("Recommended_to_Others")%><br/><% If  Rs3("Recommended_to_Others") = "Yes" Then%> <Textarea name="Recommended_to_Others" rows="4" cols="25" readonly="readonly"><% =Rs3("RO_Comments")%></Textarea><%End If%></td>
			<td style="width:250px"> <Textarea name="Completed_Cert" rows="4" cols="25" readonly="readonly"><% =Rs3("Completed_Cert")%></Textarea></td>
			
			  <td style="width:100px"> <% =Rs3("cert_ITIL")%></td>  
			    <td style="width:100px"> <% =Rs3("cert_PMP")%></td>  
				  <td style="width:100px"> <% =Rs3("cert_Scrum")%></td>  
		   <td style="width:250px"> <Textarea name="Pursue_Cert" rows="4" cols="25" readonly="readonly"><% =Rs3("Pursue_Cert")%></Textarea></td>		
			
            <td style="width:250px"> <% = Rs3("Coporate_Contact_Person")%><br/><% If  Rs3("Coporate_Contact_Person") = "Yes" Then%> <Textarea name="CP_Comments" rows="4" cols="25" readonly="readonly"><% =Rs3("CP_Comments")%></Textarea><%End If%></td>
		...
		If Rs3("Feedback_Recieved") = "Yes" Then
		...
		If  TotalCount1 > 0 Then
		  ...
		  avgCourseMaterial = Round(Course_Material / TotalCount1,2)
		  
		  Teacher_Meth = Teacher_Meth + Rs3("Rating_Teacher_Meth")
		  avgTeacherMeth = Round(Teacher_Meth / TotalCount1,2)
		   
		  Instructor = Instructor + Rs3("Rating_Instructor")
		  AvgInstructor = Round(Instructor / TotalCount1,2)
		  
	      OverallProgram = OverallProgram + Rs3("Rating_OverallProgram")
		  avgOverallProgram = Round(OverallProgram / TotalCount1,2)
		  
		  Rating = Rating + Rs3("Avg_Rating")	
	      AverageRating = Round(Rating / TotalCount1,2) 
...
-->			<td style="width:100px"><input type ="checkbox" name="guarante_class" value="Y" /></td>
			<td style="width:250px"><Textarea name="impFactors_Others" rows="5" cols="25" ></Textarea></td>
			
			<!--'form to enrolling for classroom training-->
			<td style="width:150px"><input type ="checkbox" name="Free_chapterTest" value="Y" /></td>
			<td style="width:150px"><input type ="checkbox" name="Free_foundExam" value="Y" /></td>
			<td style="width:150px"><input type ="checkbox" name="Free_guides" value="Y" /></td>
			<td style="width:150px"><input type ="checkbox" name="past_students" value="Y" /></td>
			<td style="width:150px"><input type ="checkbox" name="reviews_internet" value="Y" /></td>
			<td style="width:150px"><input type ="checkbox" name="visted_facebook" value="Y" /></td>
			<td style="width:150px"><input type ="checkbox" name="twitter_updates" value="Y" /></td>
			<td style="width:250px"><Textarea name="before_enrollothers" rows="5" cols="25" ></Textarea></td>
...
<td style="width:100px"><input type ="checkbox" name="cert_ITIL" value="Y" /></td>
<td style="width:100px"><input type ="checkbox" name="cert_PMP" value="Y" /></td>
<td style="width:100px"><input type ="checkbox" name="cert_Scrum" value="Y" /></td>			
...
<td colspan="26" align="right"><b>Average:</b></td>
...
<tr><td colspan="2"><b>Count of each Other PRINCE2 training course providers</b></td></tr>
<%
...
<td>
<table cellpadding="8" cellspacing="0" border="1" >
<tr><td colspan="2"><b>Count of each How did you here about PROJstudy</b></td></tr>
