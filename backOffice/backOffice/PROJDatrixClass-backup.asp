...
'Sql Query For Retrive The course Details'
    ...
    If session("brand") = "Datrix"  then
	  ...
	  strQuery = "SELECT courseid,courseName,city,startdate,enddate,status,coursetype,certificateId from datrixClasses WHERE country = '"& session("country") &"' AND  startdate < '"& Now() &"' and vendorId = '"&Session("vendor")&"' order by startdate desc"
		  
	
	ElseIf session("brand") = "PROJstudy" then
  ...
  ElseIf session("brand") = "All" And session("certificate") = "" then
		  ...
		  strQuery = "SELECT courseid,courseName,city,startdate,enddate,status,coursetype,certificateId from course WHERE country = '"& session("country") &"' AND  startdate < '"& Now() &"'"
		  
		  	strQuery = strQuery &" UNION ALL "
		
		strQuery = strQuery & "SELECT courseid,courseName,city,startdate,enddate,status,coursetype,certificateId from datrixClasses WHERE country = '"& session("country") &"' AND  startdate < '"& Now() &"'  order by startdate desc"
 ...
 If session("brand") = "Datrix" then
   ...
   strQuery = "SELECT courseid,courseName,city,startdate,enddate,status,coursetype,certificateId from datrixClasses WHERE country = '"& session("country") &"' AND  startdate >= '"& Now() &"' and vendorId = '"&Session("vendor")&"' order by startdate asc"
...
ElseIf session("brand") = "All" And session("certificate") = "" then
		...
		strQuery = strQuery & "SELECT courseid,courseName,city,startdate,enddate,status,coursetype,certificateId from datrixClasses WHERE country = '"& session("country") &"' AND  startdate >= '"& Now() &"'  order by startdate asc"
		 
			 
ElseIf session("brand") = "All" And session("certificate") <> "" then
					
		 strQuery = "SELECT DISTINCT courseid,courseName,city,startdate,enddate,status,coursetype,certificateId from course WHERE country = '"& session("country") &"' And startdate >= '"& Now() &"'"
		 
		 strQuery = strQuery &" UNION ALL "
		
		strQuery = strQuery & "SELECT DISTINCT courseid,courseName,city,startdate,enddate,status,coursetype,certificateId from datrixClasses WHERE country = '"& session("country") &"' And startdate >= '"& Now() &"' order by startdate asc"
...
<div id="htMap">
  ...
  <% elseIf session("certificate") <> ""  Then %>
    <tr id="htMap">
      <td  height="25px" > <% If session("brand") <> "All" Then %>  <% If Session("typeofuser") <> "CSR" Then %><a href="courses.asp?type=add&certificate=<% = rqCertificate %>">Add Class </a> |  <% End If %><% End If %> <a href="courses.asp?Course=PastCourses&certificate=<% = rqCertificate %>">Past Classes</a> | <a href="courses.asp?Course=FutureCourses&certificate=<% = rqCertificate %>">Future Classes</a> | <a href="courses.asp?type=selectcourse"><span style="color:#FF3300">Change certification type</span></a></td>
    </tr>
      ...
      <td align="left" height="200px"><form name="addcourse" action="addEditDeleteCourse.asp" method="post" onsubmit="return Form_Validator(this)">
       ...
       <%If session("brand") = "Datrix" Then %>  
        <table border="0" cellpadding="5" width="1400px" align="left" cellspacing="6">
         <%Else%>  
        <table border="0" cellpadding="5" width="100%" align="left" cellspacing="6">
         <% End If %>  
      <tr>
...
'And Display That Particular Details'
 ...
 If rqType = "Edit" Then %>
  <tr>
    <td  height="200px"><form name="addcourse" action="addEditDeleteCourse.asp" method="post" onsubmit="return Form_Validator1(this)">
       ...
       <%If session("brand") = "Datrix" Then %>  
        <table border="0" cellpadding="5" width="1300" align="left" cellspacing="6">
         <%Else%>  
        <table border="0" cellpadding="5" width="100%" align="left" cellspacing="6">
         <% End If %> 
 ...
 <% ElseIf session("brand") = "Datrix" Then %>  
    <tr>
		  ...
		  Sql1 = "select * from datrixCourse order by courseid desc"
 ...
 %>
  ...
  %> 
      ...
      <% If session("brand") = "All" Then %>  
               ...
               <% If session("certificate") = "ITIL"  Then %>  
              ...
              <% Else %>
               			...
               			<a href="courses.asp?CourseId=<% = courseId %>&certificate=<% = rqCertificate %>&city=<% = arrAllRecords(2,rowcounter) %>&startdate=<% = arrAllRecords(3,rowcounter) %>&enddate=<% = arrAllRecords(4,rowcounter) %>">
                        <% = FormatDateTime(arrAllRecords(3,rowcounter),1) %>
                        to
