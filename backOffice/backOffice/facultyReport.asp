...
End If
 ...
 'query to retrive instructor details
   strQuery = "SELECT instructorid, firstname,lastname from instructor WHERE country = '"& session("country") &"'"
...
'Query to retrive faculty name
 strQuery1 = "select firstname,lastname from instructor where instructorid = '"&rqfacultyid&"' "
...
'query to retrive city,fromdate ,to date and rating 
   strQuery2 = "SELECT c.courseid,c.city,c.startdate,c.enddate,c.instructor,avg(f.Rating_instructor) FROM course c,feedbacks f WHERE c.courseid = f.Course_ID and c.instructor ='"&rqname&"' and c.startdate between '"& rqfrom  &"' and '"&rqto&"' and f.Rating_instructor <> '0' group by c.courseid,c.city,c.startdate,c.enddate,c.instructor"
...
If rqfromReport <> "" Then
 strQuerydate = "Select c.courseid, c.City,c.startdate,enddate,c.instructor,avg(f.Rating_Instructor) FROM course c,feedbacks f WHERE c.courseid = f.Course_ID and c.startdate between '"& Reporfrom &"' and  '"&reportto&"' and f.Rating_instructor <> '0' group by c.courseId,c.instructor,c.City,c.startdate,c.enddate"
...
If rqfromMonth <> "" Then 
strQueryMonth = "Select c.instructor,avg(f.Rating_instructor) FROM course c,feedbacks f WHERE c.courseid = f.Course_ID and (CONVERT(varchar(2), DATEPART(month,c.startdate)) = '"& mont &"'  AND DATEPART(yyyy,c.startdate) = '"& yer &"') group by (c.instructor)"
...
End If
 ...
 <tr>
   <td colspan="9" height="35" class="HeaderLarge"><div align="center"><font color="#00B0EC" face="Arial" size="4">Welcome to <span style="color:#FF3300"><% =Session("country") %></span> PROJstudy Back Office</font></div></td>
 </tr>
 <tr>
