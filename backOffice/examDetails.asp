...
'======================================================='
  ...
  strQuery = "select * from liveclass_examdetails order by Examid desc"
...
'======================================================='
 ...
 If rqtype = "edit" Then
   ...
   strQuery1 = "SELECT Examid,city,examDate,examTime FROM liveclass_examdetails WHERE Examid = '" & rqExamId & "'"
...
<table border="1" height="550" cellspacing="0" class="dbborder" width="1000px">
 ...
 <tr>
  ...
  <tr>
   <td height="25"><a href="examDetails.asp?type=add">Add Exam Details</a></td>
 </tr>
 <!-- If You Click Add City That Comes Here-->
 <% If rqType = "add" Then %>
 <tr>
  <td height="200">
   <form action="addEditDelExamDetails.asp" method="post" onsubmit="return Form_Validator(this)">
 ...
 <% If rqtype = "edit" Then %>
    <tr>
     <td height="200">
       <form action="addEditDelExamDetails.asp" method="post" onsubmit="return Form_Validator1(this)">
		<table align="center" border="0">
	     <tr>
  ...
  <td valign="top">
   <table border="1" align="center" width="400px" cellspacing="0" cellpadding="2">
	...
	<tr>
	 ...
	 <td align="center"><a href="examDetails.asp?type=edit&Examid=<% = arrAllQueries(0,rowcounter)%>">Edit</a></td>
	 <td align="center"><a href="addEditDelExamDetails.asp?delete=<% = arrAllQueries(0,rowcounter)%>" onclick="return DeleteAlert(this.href);" >Delete</a></td>
	</tr>
	<% Next %>
	<tr>
	 <td colspan="8">
	   <% If Not firstRow = 0 Then %>
		<a href="examDetails.asp?page=previous&row=<% = firstRow %>"> &lt;&lt;Previous</a>
	   <% End If %>
	   <% = ((firstRow + 1) & " - " & (lastRow + 1) & " of " & (numRows + 1)) %>
	   <% If Not numRows = lastRow Then %>
		<a href="examDetails.asp?page=next&row=<% = lastRow %>"> &gt;&gt;Next</a>
	   <% End If %>
	 </td>
	</tr>
	<% End If %>
