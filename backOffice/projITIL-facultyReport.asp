...
End If
 ...
 'query to retrive instructor details
   strQuery = "SELECT instid,firstname,lastname FROM instructors"' where typeofuser='Faculty'"
...
'Query to retrive faculty name
 strQuery1 = "select firstname,lastname from instructors where instid = '"&rqfacultyid&"' "
...
'query to retrive city,fromdate ,to date and rating 
   strQuery2 = "SELECT c.courseid,c.city,c.startdate,c.enddate,c.instructor,avg(f.Rating_instructor) FROM projITIL_course c,projITIL_feedbacks f WHERE c.courseid = f.Course_ID and c.instructor ='"&rqname&"' and c.startdate between '"& rqfrom  &"' and '"&rqto&"' and f.Rating_instructor <> '0' group by c.courseid,c.city,c.startdate,c.enddate,c.instructor"
...
If rqfromReport <> "" Then
 strQuerydate = "Select c.courseid, c.City,c.startdate,enddate,c.instructor,avg(f.Rating_Instructor) FROM projITIL_course c,projITIL_feedbacks f WHERE c.courseid = f.Course_ID and c.startdate between '"& Reporfrom &"' and '"&reportto&"' and f.Rating_instructor <> '0' group by c.courseId,c.instructor,c.City,c.startdate,c.enddate"
'Response.write(strQuerydate)
   objRs1date.Open strQuerydate,ConnObj
...
If rqfromMonth <> "" Then 
strQueryMonth = "Select c.instructor,avg(f.Rating_instructor) FROM projITIL_course c,projITIL_feedbacks f WHERE c.courseid = f.Course_ID and (CONVERT(varchar(2), DATEPART(month,c.startdate)) = '"& mont &"'  AND DATEPART(yyyy,c.startdate) = '"& yer &"') group by (c.instructor)"
'Response.write(strQueryMonth)
   objRs1Month.Open strQueryMonth,ConnObj
...
End If
 ...
 <tr>
   <td colspan="9" height="35" class="HeaderLarge"><div align="center"><font color="#00B0EC" face="Arial" size="4">Welcome to <span style="color:#FF3300"><% =Session("country") %></span> PMstudy Back Office</font></div></td>
 </tr>
 <tr>
  ...
  <tr>
   <td height="25"><a href="projITIL-feedback-form.asp" target="_blank">Add Feedback</a></td>
 </tr>
  <tr>  
  <td>
  <table><tr><td>
 <!--Faculty Wise Report Table--> 
   <form name="course" action="projITIL-facultyReport.asp" method="post" onSubmit="return Form_Validator(this)">
...
<td>
<!-- Date Wise Report-->  
   <form name="date-Report" action="projITIL-facultyReport.asp" method="post" onSubmit="return Form_Validator1(this)" >
  ...
  <td> 
  <!-- Monthly Wise Report -->
  <form name="date-Report" action="projITIL-facultyReport.asp" method="post" onSubmit="return Form_Validator2(this)" >
  <table border="0" align="center" cellpadding="8" cellspacing="0" class="dbborder" height="250">
 ...
 <tr><td align="center">
  ...
  <tr class="title"><td >City: &nbsp;From Date &nbsp; To Date</td> <td>Rating</td></tr> 
   ...
   If IsArray(arrAllrecords) Then    
       ...
       %> <tr><td>
       <a href="projITIL-feedback-form.asp?courseId=<% =arrAllrecords(0,RowCounter)%>&City=<% = arrAllrecords(1,RowCounter) %>" target="_blank">
...
<% If IsArray(arrdateQueries) Then
       ...
       %>
        <tr> <td><a href="projITIL-feedback-form.asp?courseId=<% = arrdateQueries(0,dateRowCounter) %>&City=<% = arrdateQueries(1,dateRowCounter) %>" target="_blank"><% = arrdateQueries(1,dateRowCounter) %> : <% = arrdateQueries(2,dateRowCounter) %> &nbsp;<% = arrdateQueries(3,dateRowCounter) %></a>
          </td>
