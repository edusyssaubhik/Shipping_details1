...
'------------------------------------------------------'
 ...
 Dim rqStreet ,rqCity , rqState1, rqPostalCode ,rqCountry,rqtypeOfCourse,rqtypeC
 ...
 rqtypeC=Request.Form("coursetype")
 ...
 Set Rs  = Server.CreateObject("ADODB.Recordset")
  ...
  strQuery = "Select firstname, lastname, nameofemployeer, email, phoneno,street,city,state1,postalCode,country,typeOfCourse from enrolledusers  WHERE id = '"&rqEnrollUserId&"'"
  ...
  Country = Rs("country")
  rqtypeOfCourse = Rs("typeOfCourse")
  Rs.close()
...
function validate_form(thisform)
{
    with (thisform)
    {
       ...
       //Check To See If The  First Name Field Is Balnk    
 	    ...
 	    if (validate_required(employeer,"Please Enter Name of Employer")==false)
  ...
  <table border="1" width="1470px" cellspacing="0">
    <tr>
      <td colspan="3" height="35"><div align="center"><font color="#00B0EC" face="Arial" size="4">Welcome to <span style="color:#FF3300">
          <% = Session("country") %>
          </span> PROJstudy Back Office</font></div></td>
    </tr>
    <tr>
      <td width="100px" valign="top" rowspan="20"><!--#include file="includes/leftMenu.html"-->
      </td>
    </tr>
    <tr>
    <td>
    <form name="ChangeEmail" action="updateUserDet.asp?type=submit&enrollUserId=<% = rqEnrollUserId %>&courseId=<% = rqCourseId %>" method="post" onSubmit="return validate_form(this)">
...
<tr>
  ...
  <tr>
    <td width="30%"><b>Name of Employer:</b></td>
    <td><input type="text" id="employeer" name="employeer" value="<% = Employeer %>" ></input></td>
  </tr>
    <tr>
              <td width="20%" class="auto-style2"><b>Course Type :</b></td>
    <td><input type="checkbox" name="coursetype" value="P" <% if Instr(rqtypeOfCourse,"P") then %>  checked="checked" <% end if %>/> P
                <input type="checkbox" name="coursetype" value="F" <% if Instr(rqtypeOfCourse,"F") then %>  checked="checked" <% end if %>/> F
                <input type="checkbox" name="coursetype" value="C" <% if Instr(rqtypeOfCourse,"C") then %>  checked="checked" <% end if %>/> C
                &nbsp;<input type="checkbox" name="coursetype" value="RR" <% if Instr(rqtypeOfCourse,"RR") then %>  checked="checked" <% end if %>/> RR
                &nbsp;&nbsp;<input type="checkbox" name="coursetype" value="EO" <% if Instr(rqtypeOfCourse,"EO") then %>  checked="checked" <% end if %>/> EO
      </td>
            </tr>
 <tr>
...
If rqType = "submit" Then
 ...
 strEdit = "UPDATE enrolledusers SET"
 ...
 strEdit = strEdit & " typeOfCourse = '" & rqtypeC  & "'"
...
Response.Redirect("datrixClass.asp?courseId="&CourseId)
