...
Dim hb_website,rqquoteid,rqPONumber
...
If Request.Form("PONumber") <> "" Then
	rqPONumber = Request.Form("PONumber")
Else
	rqPONumber = Request.QueryString("PONumber")
End If
...
strEmpDet= "UPDATE QW_QuoteOverview  SET "
		  ...
		  If rqPONumber <> "" Then
		   strEmpDet= strEmpDet& "PONumber = '"&rqPONumber&"'"
		   End If
		  strEmpDet= strEmpDet& "status = 'P O Received'"
...
strEmpDet1= "UPDATE QW_QuoteDetails  SET "
		  ...
		  strEmpDet1= strEmpDet1& "status = 'P O Received'"
		  strEmpDet1= strEmpDet1& " WHERE quoteId = '"& rqquoteid &"'"
...
'Dim strBody
'
'strBody = "<table width=""600px"" border=""0""><tr height=""30px""><td><font face=""Arial"" size=2>Dear "&Session("sold_name1")&",<BR><BR>Greetings from Datrix Training! <BR><BR>Thank you for accepting our quotation and sending us the purchase order"
'
'If rqPOnumber <> "" Then
...
<div id="quote-wrapper">
  <div id="quote-header">
     <table width="100%" border="0" cellspacing="0" cellpadding="0" height="65px">
         <tr>
           ...
           <td valign="top"><img src="/QuoteWerks/images/po-received.jpg" align="right"/>
  ...
  <tr>
   ...
   <tr>
    <td valign="top"><% =arr1(0) %><br />
<% =arr1(1) %><br />
<% =arr1(2)  %><br />   
<% =arr1(3)  %><br />    Phone:  <%=arr1(4)%><br />Fax:<%=arr1(5)%> <br /> </td>
  
   
     <td valign="top"><% =arr2(0) %><br />
<% =arr2(1) %><br />
<% =arr2(2)  %><br />   
<% =arr2(3)  %><br />    Phone:  <%=arr2(4)%><br />Fax:<%=arr2(5)%> <br /> </td>
...
<br />
  ...
  <div style="width:auto;height:10000px; overflow:visible;" id="pdfRenderer">
      ...
      <form  action="sendPOgeneratedReport.asp"  method="post" >
      <input type="hidden" name="quoteid" value="<% = rqquoteid %>" />
    <table width="50%" align="center"  border="0" cellspacing="0" cellpadding="0" class="leftNavigation" >
    <tr><br /><br />
    <td>P O Number</td>
    <td><input type="text" name="PONumber"/>(Optional)</td></tr>
    <tr>
    <td ><br /><br /></td>
    <td ><br /><br /><button type="submit" name="type" value="submit" >Accept P.O</button></td></tr>
 </table></form>
   <%   
   Session("quote") = ""
  'response.Write(str)
