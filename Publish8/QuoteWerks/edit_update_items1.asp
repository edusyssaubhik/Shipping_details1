...
if session("new_quoteid")="" then
 ...
 strQuery = "INSERT INTO QW_QuoteDetails(quoteId,versionNo,itemName,itemCode,cost,price,quantity,actualPrice,discount,discountAmount,sellingPrice,FinalPrice,tax,shipping,totalAmount,courseid,margin,addedBy,addedDate,status,quoteno) values  ('"& Session("new_quoteid")&"',1,"&req&",'"&Session("QWUserName")&"','"& Now() &"','In Process','"&Session("quote_no")&"')"
'response.Write(strQuery)
ConnObj.Execute strQuery

'Session("quote")=""
 
  sql1="update QW_QuoteOverview set countryId='"&Session("country")&"',totalAmount='"& total &"',tax='"& tax &"' ,totalDiscount ='"& dis &"' ,totalActualPrice='"& act &"',totalsellingPrice='"& sell &"',margin_total='"& margin_total &"' where quoteId='"& Session("new_quoteid")&"'"
