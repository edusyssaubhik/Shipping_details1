...
{
    window.location = "Update_oppurtunity.aspx?qid="+x+"&query=V1";
}
function abc()
...
<% =Session("email2") %><br /> <% =Session("ShipAddress")  %> <br />    Phone:<%=Session("ph2")%>  </td>
  ...
  <tr>
    ...
    <td><%=session("DaysBefore") %> days before course, or 30 days from invoice, whichever is sooner</td>
    <td>&nbsp;</td>
  </tr>
</table>
  </div>
...
<tr><br></tr>
 <tr>
      ...
      <td width = "45%"><button type="submit" name="type" value="Submit" onClick="abc()">Save</button></td>
           <td width = "45%"><button type="submit" name="type" value="Submit" onClick="abc1(<%=Session("quote") %>)">Send</button></td>
