...
if(window.XMLHttpRequest){
 ...
 //     
       req1.onreadystatechange=onReadyState1;
    req1.open("GET","/QuoteWerks/update_items.asp?query="+new_code+"&total="+q1+"&tax="+q2+"&act="+q3+"&sell="+q4+"&dis="+q5+"&margin="+q6,true);
    req1.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
    req1.send(null);
          }
...
if(window.XMLHttpRequest){
 ...
 //
    ...
    req2.open("GET","/QuoteWerks/update_items1.asp?query="+item+"&total="+q1+"&tax="+q2+"&act="+q3+"&sell="+q4+"&dis="+q5+"&margin="+q6,true);
 ...
 if Session("quote")="" then  'should be null in first page again
  ...
  If (Not Rs1.BOF) and (Not Rs1.EOF) Then
   ...
   strEmpDet = "INSERT INTO QW_QuoteOverview(versionNo,createdDate,userId,quoteno) VALUES (1,'"& Now() &"','"&Session("QWUserName")&"','"&Session("quote_no")&"')"
	...
	SQL = "SELECT TittleDescription a, ManufacturerPartNumber b,Cost c,List d,tax t FROM [QW_category] where ManufacturerPartNumber in ("& s &")"
    ' response.Write(SQL) 
...
dim id        
   ...
   'response.Write(SQL12)
        ...
        %>
  <select id="date_loc_<%=i%>"><option value="0">---Select---</option>
<%
...
<td class="auto-style1">  <input name="qty" type="text" id="qty_<%=i %>" onBlur="calcu(<%=rs("d") %>,<%=i %>)" size="3" maxlength="8" />
