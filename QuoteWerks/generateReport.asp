...
Set objRsQuote1 = Server.CreateObject("ADODB.Recordset")
...
If rqCategory = "SalesManager" and rqUserId<>"" Then
strQuote = " And userId = '"& rqUserId &"'"
ElseIf rqCategory = "Company" and rqCompId<> "" Then 
strQuote =  " And companyId = '"& rqCompId &"'"
ElseIf  rqCategory = "Date" and rqFromDate<>""  Then 
strQuote =  " And sentDate Between '" & rqFromDate & "' AND '" & DateAdd("d", +1,CDate(rqToDate)) & "'"
Else
End If
...
End If
If rqCategory = "SalesManager" and rqUserId<>"" Then
     strQuote1 = " And userId = '"& rqUserId &"' "
     flag=1
 If rqAction = "In Process" OR rqAction = "" Then
    strQuote = "SELECT quoteId, sentDate, userId, totalAmount, margin_total, companyId, status,PONumber,quoteno,createdDate,Invoiceno,InvoiceSendDate FROM QW_QuoteOverview WHERE  status = 'In Process' "&strQuote1 &" order by createdDate desc"
    strQuote2= "SELECT sum(convert(float,margin_total)) sum_margin,sum(convert(float,totalAmount)) sum_total FROM QW_QuoteOverview WHERE  status = 'In Process' "&strQuote1 &" "    
    Else  
    strQuote = "SELECT quoteId,createdDate,userId, totalAmount, margin_total, companyId, status,PONumber,quoteno,sentDate,Invoiceno,InvoiceSendDate FROM QW_QuoteOverview WHERE status = '"& rqAction &"' "&strQuote1 &" order by sentDate desc"
    strQuote2= "SELECT sum(convert(float,margin_total)) sum_margin,sum(convert(float,totalAmount)) sum_total FROM QW_QuoteOverview  WHERE status = '"& rqAction &"' "&strQuote1 &" "
 
      end if
 
ElseIf rqCategory = "Company" and rqCompId<> "" Then 
strQuote1 =  " And companyId = '"& rqCompId &"'"
    flag=1 
    If rqAction = "In Process" OR rqAction = "" Then
    strQuote = "SELECT quoteId, sentDate, userId, totalAmount, margin_total, companyId, status,PONumber,quoteno,createdDate,Invoiceno,InvoiceSendDate FROM QW_QuoteOverview WHERE  status = 'In Process' "&strQuote1 &" order by createdDate desc"
    strQuote2= "SELECT sum(convert(float,margin_total)) sum_margin,sum(convert(float,totalAmount)) sum_total FROM QW_QuoteOverview  WHERE  status = 'In Process' "&strQuote1 &" "
...
ElseIf  rqCategory = "Date" and rqFromDate<>""  Then 
strQuote1 =  " And sentDate Between '" & rqFromDate & "' AND '" & DateAdd("d", +1,CDate(rqToDate)) & "'"
  flag=1
 If rqAction = "In Process" OR rqAction = "" Then
    strQuote = "SELECT quoteId, sentDate, userId, totalAmount, tax, companyId, status,PONumber,quoteno,createdDate,Invoiceno,InvoiceSendDate FROM QW_QuoteOverview WHERE  status = 'In Process' "&strQuote1 &" order by createdDate desc"
    strQuote2= "SELECT sum(convert(float,margin_total)) sum_margin,sum(convert(float,totalAmount)) sum_total FROM QW_QuoteOverview WHERE  status = 'In Process' "&strQuote1 &" "

 
    Else  
    strQuote = "SELECT quoteId,createdDate,userId, totalAmount, tax, companyId, status,PONumber,quoteno,sentDate,Invoiceno,InvoiceSendDate FROM QW_QuoteOverview WHERE status = '"& rqAction &"' "&strQuote1 &" order by sentDate desc"
   strQuote2= "SELECT sum(convert(float,margin_total)) sum_margin,sum(convert(float,totalAmount)) sum_total FROM QW_QuoteOverview  WHERE status = '"& rqAction &"' "&strQuote1 &" order by sentDate desc"
 
     end if

End If
    dim total_margin,total_grant
    total_margin=""
    total_grant=""
    if flag=0 then
    If rqAction = "In Process" OR rqAction = "" Then
     strQuote = "SELECT quoteId, sentDate, userId, totalAmount, tax, companyId, status,PONumber,quoteno,createdDate,Invoiceno,InvoiceSendDate FROM QW_QuoteOverview WHERE  status = 'In Process' "
     strQuote = strQuote & Search(rqCategory)& " order by createdDate desc"
    Else  
    strQuote = "SELECT quoteId,createdDate,userId, totalAmount, tax, companyId, status,PONumber,quoteno,sentDate,Invoiceno,InvoiceSendDate FROM QW_QuoteOverview WHERE status = '"& rqAction &"' "
    strQuote = strQuote & Search(rqCategory) & " order by sentDate desc"
    end if
    Else
    objRsQuote1.Open strQuote2, ConnObj
   If Not objRsQuote1.EOF Then
    total_margin=objRsQuote1("sum_margin")
    total_grant=objRsQuote1("sum_total")
   End If
    objRsQuote1.Close
   End If 
...
<script language ="JavaScript" type="text/javascript">
    ...
    function check2() {
        ...
        var f = document.getElementById("CompId").value
        if (f == "select") {
...
<script language="javascript">
    ...
    function searchby() {
       // var x = document.getElementById("action1").value;
        //alert(x)
       //window.location= "generateReport.asp?action="+x;
...
<tr>
  ...
  <div id="styledForm" class="myform">
    <table  border="0" cellspacing="0" cellpadding="4" align="left">
      <tr>
        <td><form action="generateReport.asp?action=<%=rqAction %>" name="myform" method="post" onSubmit="return Form_Validator(this)">
            <table width="40%" border="0" cellspacing="0" cellpadding="8px">
              <tr>
                <td>Search By Category:<input type="hidden" value="<%=rqAction %>" id="action1" /></td>
          ...
          <% If rqCategory = "SalesManager" Then %>
          <form action="generateReport.asp?action=<%=rqAction %>" method="post" >
            <table width="40%" border="0" cellspacing="0" cellpadding="8px">
              <tr><br>
              ...
              <tr>
                ...
                <td><select name="userId" id="userId" onChange="check1()" >
                    <option value="select">--Select--</option>
                      <% If Session("QWTypeofuser") = "Admin" Then %>
                    <% while not objRsuserId.EOF %>
                ...
                <input type="Submit" name="Search" value="Search" onClick="return check1();" /></td>
              </tr>
            </table>
          </form>
          <% ElseIf rqCategory = "Company" Then  %>
          <form action="generateReport.asp?action=<%=rqAction %>" method="post" onSubmit="return Form_Validator2(this)">
          <table width="50%" border="0" cellspacing="0" cellpadding="8px">
            ...
            <tr>
              ...
              <td><select name="CompId" id="CompId" onChange="check2()">
                  <option value="select">--Select--</option>
                  <% while not objRsComp.EOF %>
              ...
              <input type="Submit" name="Search" value="Search" onClick="return check2();" /></td>
            </tr>
          </table>
          </form>
          <% ElseIf rqCategory = "Date" Then  %>
          <form action="generateReport.asp?action=<%=rqAction %>" method="post" onSubmit="return Form_Validator3(this)">
            <table width="40%" border="0" cellspacing="0" cellpadding="8px">
              <tr><br>
...
<fieldset>
    <table>
        ...
        <%
            ...
            If rqCategory = "SalesManager" and rqUserId<>"" Then
%>
            <td colspan="3" class="auto-style1">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Total Margin:&nbsp;<%=total_margin %> </td><td>Total Amount :<%=total_grant %></td>
...
End If
        ...
        <table border="0" cellpadding="0" cellspacing="0" class="table1" width="950px">
          <tr>
           ...
           <td  align="center"><b>Quote Number</b></td>
              <%
                  If rqAction = "In Process" OR rqAction = "" Then%>
            <td  align="center"><b>Create Date</b></td>
                  <%else  %>
            <td  align="center"><b>Send Date</b></td>
              <% end if %>
          ...
          <% End If %>
             ...
             quoteno=arrAllRecords(8,rowcounter)
             createdate=arrAllRecords(9,rowcounter)
             InvoiceNo= arrAllRecords(10,rowcounter)
	         InvoiceSendDate=arrAllRecords(11,rowcounter)
                 	 %>
          <tr>
             <td align="center"><% = userId %></td>
             <td  align="center"><% = quoteno %></td>
              <% if InvoiceNo<>""  and InvoiceNo<>"NULL"  then %>
               <td  align="center"><% = InvoiceSendDate %></td>
              <% else %>
              ...
              <td><br />
                ...
                <select name="status" id="status_<%=index %>" onChange="visible_text(<%=index %>)">
                  ...
                  <% If quoteStatus <> "Quote Cancelled" Then %>                    
                    <option value="Quote Cancelled">Quote Cancelled</option>
				  <% End If %>                    
                </select>
                <br />
                <br />
          ...
          <div style="float:left; width: 207px;">
            <label id="la_<%=index %>" style="display:block; width: 55px; float: left;" >PO # : </label><input id="text_<%=index %>" name="text_<%=index %>" type="text" style="display:block" value="<%=arrAllRecords(7,rowcounter) %>"   /></div>
             <br />    <input type="submit" name="type" value="Submit" />
                   <%If quoteStatus <> "P O Received" or quoteStatus <> "Invoice Sent" or quoteStatus <> "Payment Received" then
                  if arrAllRecords(10,rowcounter)<>"" then
                     
                      %>
                  <label>Invoice No# <%=arrAllRecords(10,rowcounter) %>
                                                                         </label><% end if 
                                                                             end if%></td>
            </form>
            <td><ul>
            <% If quoteStatus = "Quote Sent" OR quoteStatus = "P O Received" OR quoteStatus = "Invoice Sent" or quoteStatus="Payment Received" Then %>
            <li><a href="/Quotewerks/ViewEditQuote.asp?action=View&id=<% = quoteId %>&req=<%=userId %>" target="_blank">View Quote</a></li>
                  <% End If %>
           
             <% If quoteStatus = "P O Received" OR quoteStatus = "Invoice Sent" OR quoteStatus = "Payment Received"  Then %>
           <li><a href="/Quotewerks/ViewEditQuote.asp?action=View_PO&id=<% = quoteId %>&req=<%=userId %>" target="_blank">View P.O</a></li>
          
                  <% End If %>
            
             <% If quoteStatus = "Invoice Sent" OR quoteStatus = "Payment Received" Then %>
            <li><a href="/Quotewerks/ViewEditQuote.asp?action=View_IS&id=<% = quoteId %>&req=<%=userId %>" target="_blank">View Invoice</a></li>
          
                  <% End If %>
             <% if quoteStatus = "Payment Received" Then %>
            <li><a href="/Quotewerks/send_payment_report.aspx?id=<%=quoteId %>&pono=<%=arrAllRecords(7,rowcounter) %>" target="_blank">send payment receipt
</a></li>
            <% End If %>
    
             <% invoice=arrAllRecords(10,rowcounter)
                 if quoteStatus = "P O Received"  Then 
                 if invoice<>"" then
                else

                  %>
         <li><a href="/Quotewerks/generate_invoice.aspx?id=<%=quoteId %>&pono=<%=arrAllRecords(10,rowcounter) %>&quoteStatus=Invoice Sent&user=<%=qwuser %>" >Generate Invoice
</a></li>
            <% End If 
                end if%>
          ...
          <td><ul><li><a href="http://projstudy.com/datrix-enrollment.asp?QuoteID=<% = quoteId %>" target="_blank">Enroll</a></li>
          <li><a href="http://projstudy.com/datrix-enrollment.asp?State=View&QuoteID=<% = quoteId %>" target="_blank">View Enrolled Users</a></li>
          <li><a href=" http://projstudy.com/backOffice/courses.asp?type=add&brand=Datrix" target="_blank">Add Onsite Classes</a></li>
          </td>  
            <% End If %>
