...
objRs2.Open strEmpDet, ConnObj
  ...
  while not objRs2.EOF 
         quoteno=objRs2("quoteno")
          ...
          InvoiceSendDate=objRs2("InvoiceSendDate")
...
Our terms of business can be viewed at:<a href="http://www.datrixtraining.com/about/terms/">http://www.datrixtraining.com/about/terms/</a>
     <% if rqAction = "View_IS" Then
 ...
 <div id="" style="height:132px !important;">
  ...
  <% End If %>
    ...
    <input type="hidden" name="redirecturl" value="ViewEditQuote.asp?action=<% = rqAction %>&id=<% = rqId%>&req=<%=user_id%>" >
       <% if rqAction<>"View_PO" then %>
        <input type="submit" name="type" value="Send <%=button_name %>" /><% end if %>
     </form>
   </div>
...
strEditDetail="select distinct quoteno AS a from QW_QuoteOverview where userId = '"&Session("QWUserName")&"'"
      ...
      <tr>
        ...
        <td width="15%" ><% = objRs22("status") %></td>
         <td width="15%">
          ...
          <a href="/Quotewerks/ViewEditQuote.asp?action=Edit&amp;id=<% = objRs22("quoteId") %>">Edit</a>/ <% end if %><a href="/Quotewerks/ViewEditQuote.asp?action=View&amp;id=<% = objRs22("quoteId") %>">View</a></td>
