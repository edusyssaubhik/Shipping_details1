...
{
window.location="Choose_option.asp?qid="+x;
}
function abc()
{
...
If Request.Form("req") <> "" Then
	user_id = Request.Form("req")
ElseIf Request.QueryString("req") <> "" Then
	user_id = Request.QueryString("req")
Else
'user_id=request.QueryString("userid")
...
strEmpDet = "SELECT * FROM QW_QuoteOverview  where quoteId = '"&rqId&"' and userId = '"&user_id&"' order by versionNo desc "
...
strEmpDet = "SELECT * FROM users where userId = '"&user_id&"' "
...
<tr>
  ...
  <div id="styledForm" class="myform" align="center">
...
<div id="quote-wrapper">
  ...
  <%
     ...
     id1= user_id
    userimage="/QuoteWerks/UserImage/"&id1&"/"&id1&"_image.jpg"  
     usersign="/QuoteWerks/UserSign/"&id1&"/"&id1&"_sign.jpg"  
	
  %>
   <tr>
<td valign="top"><%=arr1(1) %><br /><%=arr1(2) %><br /><%=arr1(3)%><br />phone: <%=arr1(1) %> </td>
    <td valign="top"><%=arr2(1) %><br /><%=arr2(2) %><br /><%=arr2(3) %><br />Phone: <%=arr2(4) %> </td>
...
Our terms of business can be viewed at:<a href="#">http://www.datrixtraining.com/about/terms/</a>
     <% if rqAction = "View_IS" Then
...
<div id="quote-wrapper">
  <div id="quote-header">
     <table width="100%" border="0" cellspacing="0" cellpadding="0" height="65px">
         <tr>
           <td width="210px" valign="top"><img src="/QuoteWerks/images/logo.jpg" /></td>
           <td width="420px" valign="top">N1 Chorley Business & Technology Centre Euxton Lane, Chorley PR7 6TE</br> Phone: 0800 7810626 Fax: 01257270478</td>
           <td valign="top">
               <% 
                   dim button_name
                   button_name=""
                   ...
                   <%
                       elseif rqAction = "View_PO" then
                       button_name="PO" 
                       %>
                 <img src="/QuoteWerks/images/po-received.jpg" align="right"/>
               <%
                       elseif rqAction = "View_IS" then
                   button_name="Invoice"    
                    %>
  ...
  <tr>
      <%if rqAction = "View_IS" Then
           if Invoiceno<>"" then%>
      <td style="text-align:right; padding-top:6px;"><b>Invoice Number</b></td>
      <td style="text-align:right; padding-top:6px;"><b><% =Invoiceno%></b></td>
 <%  end if
     else %>
    <td style="text-align:right; padding-top:6px;"><b>Quote Number</b></td>
    <td style="text-align:right; padding-top:6px;"><b><% =quoteno%></b></td>
 <% 
  ...
  %>
   <tr>
    <td valign="top"><%=arr1(1) %><br /><%=arr1(2) %><br /><%=arr1(3) %> <br />   Phone: <%=arr1(4) %> </td>
    <td valign="top"><%=arr2(1) %><br /><%=arr2(2) %><br /><%=arr2(3) %>   <br /> Phone: <%=arr2(4) %> </td>
    <td valign="top" class="small-text"><b>To accept this quotation, please tick
    required line items, enter a PO number,
    sign & date below and return.</b><br /><br />
    ...
    <b>Please email us if your organisation does
    not use PO numbers.</b></td>
   <td valign="top" align="left"><div style="height: 112px"><div style="float:left; width: 75px; height: 89px;"><img src="/QuoteWerks/UserImage/<%=id1 %>/<%=id1 %>_image.jpg" style="height: 97px; width: 78px" /></div><div style="float:right; height: 93px">
          
            <b><% = rqfirstname%>&nbsp;<%= rqlastname%></b> <br /><br /> <b><% =rqofficialemailid %></b> <br /><br /> <b><% =rqphoneno %> </b><br /><br />
            <img src="/QuoteWerks/UserSign/<%=id1 %>/<%=id1 %>_sign.jpg" style="width: 108px; height: 30px" /></div></div></td>
 </tr>
</table>
    <table width="100%" border="0" cellspacing="0" cellpadding="0" style=" margin-top: -13px;">
  <tr>
    <td class="titlebackground" width="70"  height="18"><b>P.O. Number</b></td>
...
removed from overdue invoices.</b>
 ...
 <div id="" style="height:132px !important;">
   
   
    <div id="Div12">
      
   </div>
   <% If rqAction <> "View_PO" or rqAction <> "View_IS" Then  %>
   
    <div  style="margin-top: -50px;float:inherit;text-align:center;margin-left:291px;">
     
         &nbsp;    &nbsp;  
  <center>   
      <% if rqAction <> "View_IS" then %>
  <input type="button" name="I accept" value="I Accept" style="width:200px;text-align:center;"/></center></div>
  <br /><br />
  <% end if%>
  <% End If %>
   
    <div id="Div14" style="float:right;">
    ...
    <input type="hidden" name="redirecturl" value="ViewEditQuote.asp?action=<% = rqAction %>&id=<% = rqId%>&req=<%=user_id%>" >
...
Our terms of business can be viewed at:<a href="#">http://www.datrixtraining.com/about/terms/</a> 
     <% if rqAction = "View_IS" Then
      ...
      Datrix Training Limited 
   <% end if %>
     - Company number 4344412, VAT registration id: GB 117 6616 13
...
strEditDetail="select distinct quoteno AS a from QW_QuoteOverview where userId = '"&user_id&"'"
...
While Not objRs11.EOF
    arr11(i)=objRs11("a")
   ' response.Write(arr11(i))
    i=i+1
    
    objRs11.Movenext
    wend
	objRs11.Close
	'response.Write(UBOUND(arr11))
      'response.Write(i)
         'j=i-1
    ...
    ' while j<> 0      
    strEdit="select top 1 * from QW_QuoteOverview where quoteno= '"&arr11(j)&"' order by quoteId desc "
