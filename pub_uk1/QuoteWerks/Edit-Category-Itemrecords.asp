...
<link href="/QuoteWerks/css/form1.css" rel="stylesheet" type="text/css" />
...
var req2=null;
    ...
    */
    ...
    { //need all checking
        //alert("ddd")
        var c,f, m,h,k,l,sell,final,tax,margin,margin1;
        var cost,cost1
        cost='cost_'+y
        margin='margin_'+y
        var sell='sell_'+y;
        ...
        if(c!="" && f!="" && qty1!="" ){
            if(int_check(f) && int_check(qty1)){
                ...
                var cal1=parseFloat(document.getElementById(sell).value)
                var cal2=parseFloat(document.getElementById(cost).value)
                var cal4=parseFloat(qty1)
                var cal3=parseFloat(cal1-cal2*cal4);
                document.getElementById(margin).value=cal3;
                if(tax_x!="")
    ...
    function add_margin_b1()
    {
        var add1=0.0;
        var count=document.getElementById("count").value;
        document.getElementById("t_margin").innerHTML="";
        for(var i=1;i<=count;i++)
        {
            var t='margin_'+i;
		
            var act_price=document.getElementById(t).value
            if(act_price!="")
            {
        ...
        document.getElementById("t_margin").innerHTML=add1;

    }

         
    function add_cost_b1()
    {
        var add1=0.0;
        var count=document.getElementById("count").value;
        //alert(count)
        document.getElementById("t_cost").innerHTML="";
        for(var i=1;i<=count;i++)
        {
            var t='cost_'+i;
        ...
        add1=parseFloat(add1).toFixed(2)
        document.getElementById("t_cost").innerHTML=add1;
    ...
    function back_cal(y)
    {
        ...
        // sell_1 //selling pr
        // act_1//actual price
        var c,f, m,h,k,l,sell,sell1,final,tax,margin;
...
form.setAttribute("action", "Edit-Category-Item.asp");
...
function take_backup()
{
    var c,f, m,h,k,l,sell,final,tax,ship,h1,new_code,q1,q2,q3,q4,q5,q6,margin;
 ...
 */   
	...
	//	var sell='sell_'+y;
		...
		var margin=parseFloat(document.getElementById(margin1).value)
		var courseid=document.getElementById(date_loc1).value
		querystring[i-1]="'"+item1+"','"+code1+"','"+cost1+"','"+price1+"',"+qty+",'"+ac_p+"','"+dis+"','"+di+"','"+sel+"','"+fin+"','"+tax_x+"','"+ship+"','"+total_price+"',"+courseid+",'"+margin +"'";
   ...
   q2=add_tax_price();
	...
	q6=document.getElementById("t_margin").innerHTML
		if(count>1)
		{		
...
if(window.XMLHttpRequest){
 ...
 //
       req1.onreadystatechange=onReadyState1;
       req1.open("GET","/QuoteWerks/edit_update_items.asp?query="+new_code+"&total="+q1+"&tax="+q2+"&act="+q3+"&sell="+q4+"&dis="+q5+"&margin="+q6,true);
    req1.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
    req1.send(null);
          }
...
var req5=null;
function call_CRM(x,y) 
{
    //window.location="crm_edit_Quote.aspx?query="+x+"&opt="+y;
   if(window.XMLHttpRequest){
    ...
    //define callback handler
    if(req5) {
        //call CRM connector page
        req5.onreadystatechange=onReadyState12;
        req5.open("GET","/QuoteWerks/crm_edit_Quote.aspx?query="+x+"&opt="+y,true);
...
function onReadyState12()
{
    var ready1=req5.readyState;
    var data=null;
    if(ready1==4){ 
 //CRM connection got closed and data is updated 
        //document.write(req2.responseText)
        //check ready state
        // data=req2.responseText;
//        alert("Thank You!Quote is Updated in CRM");
        var x= <%= Session("qid_edit") %>
        var usr="<%=Session("QWUserName")%>"
        var que3="<%=Session("QWTypeofuser") %>"
      
        window.location="new_ship_edit.aspx?query=V2&qid="+x+"&que1=QWEEDNAAAAAAAAAAAAAAAAAAAAAAANMMNMLKLNKLLKNLKKLOOOOOOOOMLMLL&que2="+usr+"&que3="+que3;
...
function sendreq1(item,q1,q2,q3,q4,q5,q6)
{
...
if(window.XMLHttpRequest){
 ...
 //
    ...
    req2.open("GET","/QuoteWerks/edit_update_items1.asp?query="+item+"&total="+q1+"&tax="+q2+"&act="+q3+"&sell="+q4+"&dis="+q5+"&margin="+q6,true);
...
// data=req2.responseText;
     var x= <%= Session("qid_edit") %>
     var usr="<%=Session("QWUserName")%>"
     var option= "<%=session("opt")%>"
   
     call_CRM(x,option);

     /*  var form = document.createElement("form");
      form.setAttribute("method", "post"); 
      form.setAttribute("action", "ship_edit/new_ship_edit.aspx");
     ...
     //window.location="new_ship_edit.aspx?Session("qid_edit")"; 
 // alert("hello")
 //document.write(data)
 }
 }
...
//     window.location = "new_ship_edit.aspx?";
 //document.write(data)
...
if s="" then
 ...
 if  Session("qid_edit")<>"" then  'should be null in first page again
...
Set  Rs2 = Server.CreateObject("ADODB.Recordset")
  ...
  s12="SELECT itemCode b FROM  QW_QuoteDetails where quoteId='"&Session("qid_edit")&"'"
 ' response.Write(s12)
...
SQL="SELECT itemName a ,itemCode b,courseid id,cost c,price d,quantity e,actualPrice f,discount g ,discountAmount h,sellingPrice i,FinalPrice j,tax k,shipping l,margin m FROM QW_QuoteDetails where quoteId='"&Session("qid_edit")&"' and itemCode  in ("& common_item &")"
rs.open SQL, ConnObj
While Not rs.EOF
i=i+1 
j=j+1
...
<td><%=rs("b")%><input type="hidden" id="code_<%=i%>" value="<%=rs("b")%>" /></td>
  <td> 
   <%
dim id,id1        
   SQL12 = "select b.courseid id,b.city ci,b.startdate st,b.enddate ed from QW_category a,datrixClasses b  where b.Refnumber=a.ManufacturerPartNumber and b.Refnumber ='"&rs("b")&"' and  b.startdate  BETWEEN  getdate()-1 and b.enddate+1"
   'response.Write(SQL12)
       k=0
        %>
  <select id="date_loc1_<%=i%>"><option value="0" >---Select---</option>
<%
    ...
    rs9.MoveNext
      ...
      ' response.Write(k)
       if k=0 then
          %>
     <option value="0" selected="selected">Not finialized</option>
<%end if
 rs9.Close         
...
<td> 	
 <!--<select name="tax" id="tax_<%=i%>" onChange="load_final_price(this.value,<%=i%>)" >
		 ...
		 <option  value="0"  <% If rs("k") = "0.00"  Then %> selected="selected" <% End If %> >0%</option>
		 <option  value="20" <% If rs("k") = "20.00"  Then  %> selected="selected" <% End If %>>20%</option>
	  </select> -->
   <input type="text" id="tax_<%=i%>" name="tax" value="<%=rs("k") %>" disabled="disabled" onChange="load_final_price(this.value,<%=i%>)"/>
...
<td><%=rs("c")%><input type="text" style="display:none" id="cost_<%=i %>" value="<%=rs("c")%>" /></td>
<td><input type="text" name="margin_<%=i %>" id="margin_<%=i %>" value="<%=rs("m") %>"   disabled="disabled" /></td>
...
'response.Write(uncommon_item)
 SQL1 = "SELECT TittleDescription a, ManufacturerPartNumber b,Cost c,List d,tax t FROM [QW_category]  where ManufacturerPartNumber in ("&  uncommon_item&")"
' response.Write(SQL1)
...
<%
  ...
  '      response.Write(k)
       if k=0 then
          %>
     <option value="0" selected="selected">Not Finalized</option>
<%end if
 rs9.Close         
...
<td> 	
 ...
 <input type="text" id="tax_<%=i%>" name="tax" value="<%=x %>" disabled="disabled" onChange="load_final_price(this.value,<%=i%>)"/>
...
<td><%=rs("c")%><input type="text" style="display:none" id="cost_<%=i %>" value="<%=rs("c")%>" /></td>
<td><input type="text" name="margin_<%=i %>" id="margin_<%=i %>"   disabled="disabled" /></td>
...
'end if
 ...
 <td><span id="t_grant">0.00</span></td>
      ...
      <td>Total Cost =</td>
 <td><span id="t_cost">0.00</span></td>
 </tr>
<tr>
 <td>Total Margin  =</td>
 <td><span id="t_margin">0.00</span></td>
