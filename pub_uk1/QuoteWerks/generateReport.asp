...
If rqCategory = "SalesManager" and rqUserId<>"" Then
 ...
 If rqAction = "In Process" OR rqAction = "" Then
    strQuote = "SELECT quoteId, sentDate, userId, totalAmount, margin_total, companyId, status,PONumber,quoteno,createdDate,Invoiceno FROM QW_QuoteOverview WHERE  status = 'In Process' "&strQuote1 &" order by createdDate desc"
    ...
    Else  
    strQuote = "SELECT quoteId,createdDate,userId, totalAmount, margin_total, companyId, status,PONumber,quoteno,sentDate,Invoiceno FROM QW_QuoteOverview WHERE status = '"& rqAction &"' "&strQuote1 &" order by sentDate desc"
...
strQuote1 =  " And sentDate Between '" & rqFromDate & "' AND '" & DateAdd("d", +1,CDate(rqToDate)) & "'"
  flag=1
 If rqAction = "In Process" OR rqAction = "" Then
    strQuote = "SELECT quoteId, sentDate, userId, totalAmount, tax, companyId, status,PONumber,quoteno,createdDate,Invoiceno FROM QW_QuoteOverview WHERE  status = 'In Process' "&strQuote1 &" order by createdDate desc"
    strQuote2= "SELECT sum(convert(float,margin_total)) sum_margin,sum(convert(float,totalAmount)) sum_total FROM QW_QuoteOverview WHERE  status = 'In Process' "&strQuote1 &" "

 
    Else  
    strQuote = "SELECT quoteId,createdDate,userId, totalAmount, tax, companyId, status,PONumber,quoteno,sentDate,Invoiceno FROM QW_QuoteOverview WHERE status = '"& rqAction &"' "&strQuote1 &" order by sentDate desc"
...
End If
    ...
    if flag=0 then
    If rqAction = "In Process" OR rqAction = "" Then
     strQuote = "SELECT quoteId, sentDate, userId, totalAmount, tax, companyId, status,PONumber,quoteno,createdDate,Invoiceno FROM QW_QuoteOverview WHERE  status = 'In Process' "
     strQuote = strQuote & Search(rqCategory)& " order by createdDate desc"
    Else  
    strQuote = "SELECT quoteId,createdDate,userId, totalAmount, tax, companyId, status,PONumber,quoteno,sentDate,Invoiceno FROM QW_QuoteOverview WHERE status = '"& rqAction &"' "
            ...
            <% End If %>
             ...
             <% invoice=arrAllRecords(10,rowcounter)
                 ...
                 if invoice<>"" then
                  ...
                  %>
         <li><a href="/Quotewerks/generate_invoice.aspx?id=<%=quoteId %>&pono=<%=arrAllRecords(10,rowcounter) %>&quoteStatus=Invoice Sent&user=<%=qwuser %>" target="_blank">Generate Invoice
</a></li>
            <% End If 
                end if%>
