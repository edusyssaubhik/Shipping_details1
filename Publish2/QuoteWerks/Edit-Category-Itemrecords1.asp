...
// data=req2.responseText;
 ...
 var x= <%=Session("quote") %>
    var usr=<%Session("QWUserName")%>
      ...
      window.location ="new_ship_edit.aspx?query=V3&qid="+x+"&ite=AAAAAAAAAQQQQQQQQQQQQQQQQQQKKKKKKKKKKKKKKKKKKKKKKKKKK&que2="+usr;
...
SQL="SELECT itemName a ,itemCode b,cost c,price d,quantity e,actualPrice f,discount g ,discountAmount h,sellingPrice i,FinalPrice j,tax k,shipping l FROM QW_QuoteDetails where quoteId='"&ses_new&"' and itemCode  in ("& common_item &")"
rs.open SQL, ConnObj
While Not rs.EOF
i=i+1 
j=j+1
