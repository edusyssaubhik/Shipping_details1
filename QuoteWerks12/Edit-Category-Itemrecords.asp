...
if(window.XMLHttpRequest){
 ...
 //
       req1.onreadystatechange=onReadyState1;
    req1.open("GET","/QuoteWerks/edit_update_items.asp?query="+new_code+"&total="+q1+"&tax="+q2+"&act="+q3+"&sell="+q4+"&dis="+q5,true);
    req1.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
    req1.send(null);
          }
...
if(window.XMLHttpRequest){
 ...
 //
    ...
    req2.open("GET","/QuoteWerks/edit_update_items1.asp?query="+item+"&total="+q1+"&tax="+q2+"&act="+q3+"&sell="+q4+"&dis="+q5,true);
...
// data=req2.responseText;
     ...
     window.location="new_ship_edit.aspx?query=V2&qid="+x+"&que1=QWEEDNAAAAAAAAAAAAAAAAAAAAAAANMMNMLKLNKLLKNLKKLOOOOOOOOMLMLL&que2="+usr;
...
<th width="144">Discount value</th>
...
SQL="SELECT itemName a ,itemCode b,cost c,price d,quantity e,actualPrice f,discount g ,discountAmount h,sellingPrice i,FinalPrice j,tax k,shipping l FROM QW_QuoteDetails where quoteId='"&Session("qid_edit")&"' and itemCode  in ("& common_item &")"
rs.open SQL, ConnObj
While Not rs.EOF
i=i+1 
j=j+1
...
<td><input type=text name="sel_price" id="sell_<%=i %>" value=<%=rs("i")%> onBlur=""  /></td>
<td> 	
...
'response.Write(uncommon_item)
 SQL1 = "SELECT TittleDescription a, ManufacturerPartNumber b,Cost c,Price d FROM [QW_category] where ManufacturerPartNumber in ("&  uncommon_item&")"
' response.Write(SQL1)
...
<td><input type=text name="sel_price" id="sell_<%=i %>" onBlur=""  /></td>
<td> 	
 <select name="tax" id="tax_<%=i%>" onChange="load_final_price(this.value,<%=i%>)" >
