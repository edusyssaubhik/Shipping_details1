 ...
 var second = "txtzone"; //id of second SELECT
 var third = "txtstate"; //id of thiredSELECT
 var forth = "txtcity"; //id of forth SELECT
 var fifth = "fixedBudget"; //id of forth SELECT
//
...
function sendRequest4(url,params,HttpMethod) {
 ...
 if(ready4==4){ //check ready state
    ...
    for(var i = 0; i < length-1; i++) {
       var childEl = document.createElement('span'); //create option
...
//
 ...
 if(selected!= 0) { //if they choose something other than the first select-->"Select topic first"
    sendRequest1('/hotelbooking/getzone.asp?countryId='+el.options[selected].value);
    ob2.disabled=0;
...
//
 ...
 if(selected!= 0) { //if they choose something other than the first select-->"Select topic first"
    sendRequest2('/hotelbooking/getstates.asp?zone='+el.options[selected].value);
    ob2.disabled=0;
	//document.write('/hotelbooking/getstates.asp?zone='+el.options[selected].value)
 } else { //otherwise add the Select Topic First option and disable it
    var childEl = document.createElement('option');
    ob2.appendChild(childEl);
    childEl.innerHTML = 'Select zone First';
    ob2.disabled=1;
 }
}
...
//
 ...
 if(selected!= 0) { //if they choose something other than the first select-->"Select topic first"
    ...
    sendRequest3('/hotelbooking/getcities.asp?state='+el.options[selected].value);
    ob2.disabled=0;
	//document.write('/hotelbooking/getcities.asp?state='+el.options[selected].value)
 } else { //otherwise add the Select Topic First option and disable it
    var childEl = document.createElement('option');
    ob2.appendChild(childEl);
 ...
 var el = document.getElementById(forth);
 var ob2=document.getElementById(fifth);
 var selected = el.selectedIndex;
//document.write(selected);
...
if(selected!= 0) { //if they choose something other than the first select-->"Select topic first"
    ...
    sendRequest4('/hotelbooking/getfixedbudget.asp?city='+el.options[selected].value);
    ob2.disabled=0;
